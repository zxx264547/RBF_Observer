...
function [centers, sigma, weights] = rbf_train(x, y, min_centers, max_centers)
    ...
    for num_centers = min_centers:max_centers
        ...
        % 添加偏置项
        Phi_train = [Phi_train, ones(n_train, 1)];
    
        % 通过最小二乘法计算权重
        weights = pinv(Phi_train' * Phi_train) * Phi_train' * y_train;
        ...
        % 添加偏置项
        Phi_test = [Phi_test, ones(n_test, 1)];
    ...
    % for i = 1:num_outputs
    %     figure;
    %     plot(1:n_test, y_test(:, i), 'b', 'LineWidth', 1.5);
    %     hold on;
    %     plot(1:n_test, y_pred(:, i), 'r--', 'LineWidth', 1.5);
    %     xlabel('样本点', 'FontSize', 12);
    %     ylabel(['输出 ', num2str(i)], 'FontSize', 12);
    %     legend('真实值', '预测值', 'FontSize', 12);
    %     title(['RBF神经网络测试结果 - 输出 ', num2str(i), ' (中心点数量: ', num2str(num_centers), ')'], 'FontSize', 14);
    %     grid on;
    %     hold off;
    % end
end